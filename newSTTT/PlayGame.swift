...
struct PlayGame {
    ...
    /* func isGridTTT(grd3: Int, tok3: String) -> Int {
         // there has to be a better way to do this
         if game[pos(pt1: [grd3, 1, 1])] == tok3 && game[pos(pt1: [grd3, 1, 2])] == tok3 && game[pos(pt1: [grd3, 1, 3])] == tok3 { // TTT in row 1
             return 1
         } else if game[pos(pt1: [grd3, 2, 1])] == tok3 && game[pos(pt1: [grd3, 2, 2])] == tok3 && game[pos(pt1: [grd3, 2, 3])] == tok3 { // TTT in row 2
             return 2
         } else if game[pos(pt1: [grd3, 3, 1])] == tok3 && game[pos(pt1: [grd3, 3, 2])] == tok3 && game[pos(pt1: [grd3, 3, 3])] == tok3 { // TTT in row 3
             return 3
         } else if game[pos(pt1: [grd3, 1, 1])] == tok3 && game[pos(pt1: [grd3, 2, 1])] == tok3 && game[pos(pt1: [grd3, 3, 1])] == tok3 { // TTT in col 1
             return 4
         } else if game[pos(pt1: [grd3, 1, 2])] == tok3 && game[pos(pt1: [grd3, 2, 2])] == tok3 && game[pos(pt1: [grd3, 3, 2])] == tok3 { // TTT in col 2
             return 5
         } else if game[pos(pt1: [grd3, 1, 3])] == tok3 && game[pos(pt1: [grd3, 2, 3])] == tok3 && game[pos(pt1: [grd3, 3, 3])] == tok3 { // TTT in col 3
             return 6
         } else if game[pos(pt1: [grd3, 1, 1])] == tok3 && game[pos(pt1: [grd3, 2, 2])] == tok3 && game[pos(pt1: [grd3, 3, 3])] == tok3 { // TTT in LR diagonal
             return 7
         } else if game[pos(pt1: [grd3, 1, 3])] == tok3 && game[pos(pt1: [grd3, 2, 2])] == tok3 && game[pos(pt1: [grd3, 3, 1])] == tok3 { // TTT in RL diagonal
             return 8
         } else { // no TTT in this grid
             return 0
         } // end if
    ...
    /*    mutating func proposeOorT(grd12: Int, tok12: String) {
         ...
         if game[pos(pt1: [grd12, 1, 1])] == tok12 && game[pos(pt1: [grd12, 1, 2])] == tok12 && game[pos(pt1: [grd12, 1, 3])] == under {
             ...
             tempRC = [3, 1]
             // proposeFlag = true
         } else if game[pos(pt1: [grd12, 1, 1])] == tok12 && game[pos(pt1: [grd12, 2, 1])] == tok12 && game[pos(pt1: [grd12, 3, 1])] == under {
             // col 1
             // 11, 21 -> 31
             tempRC = [3, 1]
             // proposeFlag = true
         } else if game[pos(pt1: [grd12, 1, 1])] == tok12 && game[pos(pt1: [grd12, 3, 1])] == tok12 && game[pos(pt1: [grd12, 2, 1])] == under {
             // col 1
             // 11, 31 -> 21
             tempRC = [2, 1]
             // proposeFlag = true
         } else if game[pos(pt1: [grd12, 2, 1])] == tok12 && game[pos(pt1: [grd12, 3, 1])] == tok12 && game[pos(pt1: [grd12, 1, 1])] == under {
             // col 1
             // 21, 31 -> 11
             tempRC = [1, 1]
             // proposeFlag = true
         } else if game[pos(pt1: [grd12, 1, 2])] == tok12 && game[pos(pt1: [grd12, 2, 2])] == tok12 && game[pos(pt1: [grd12, 3, 2])] == under {
             // col 2
             // 12, 22 -> 32
             tempRC = [3, 2]
             // proposeFlag = true
         } else if game[pos(pt1: [grd12, 1, 2])] == tok12 && game[pos(pt1: [grd12, 3, 2])] == tok12 && game[pos(pt1: [grd12, 2, 2])] == under {
             // col 2
             // 12, 32 -> 22
             tempRC = [2, 2]
             // proposeFlag = true
             ...
             tempRC = [1, 2]
             // proposeFlag = true
         } else if game[pos(pt1: [grd12, 1, 3])] == tok12 && game[pos(pt1: [grd12, 2, 3])] == tok12 && game[pos(pt1: [grd12, 3, 3])] == under {
             // col 3
             // 13, 23 -> 33
             tempRC = [3, 3]
             // proposeFlag = true
         } else if game[pos(pt1: [grd12, 1, 3])] == tok12 && game[pos(pt1: [grd12, 3, 3])] == tok12 && game[pos(pt1: [grd12, 2, 3])] == under {
             // col 3
             // 13, 33 -> 23
             tempRC = [2, 3]
             // proposeFlag = true
         } else if game[pos(pt1: [grd12, 2, 3])] == tok12 && game[pos(pt1: [grd12, 3, 3])] == tok12 && game[pos(pt1: [grd12, 1, 3])] == under {
             // col 3
             // 23, 33 -> 13
             tempRC = [1, 3]
             // proposeFlag = true
         } else if game[pos(pt1: [grd12, 1, 1])] == tok12 && game[pos(pt1: [grd12, 2, 2])] == tok12 && game[pos(pt1: [grd12, 3, 3])] == under {
             // diag L to R
             // 11, 22 -> 33
             tempRC = [3, 3]
             // proposeFlag = true
         } else if game[pos(pt1: [grd12, 1, 1])] == tok12 && game[pos(pt1: [grd12, 3, 3])] == tok12 && game[pos(pt1: [grd12, 2, 2])] == under {
             // diag L to R
             // 11, 33 -> 22
             tempRC = [2, 2]
             // proposeFlag = true
         } else if game[pos(pt1: [grd12, 2, 2])] == tok12 && game[pos(pt1: [grd12, 3, 3])] == tok12 && game[pos(pt1: [grd12, 1, 1])] == under {
             // diag L to R
             // 22, 33 -> 11
             tempRC = [1, 1]
             // proposeFlag = true
         } else if game[pos(pt1: [grd12, 1, 3])] == tok12 && game[pos(pt1: [grd12, 2, 2])] == tok12 && game[pos(pt1: [grd12, 3, 1])] == under {
             // diag R to L
             // 13, 22 -> 31
             tempRC = [3, 1]
             // proposeFlag = true
         } else if game[pos(pt1: [grd12, 1, 3])] == tok12 && game[pos(pt1: [grd12, 3, 1])] == tok12 && game[pos(pt1: [grd12, 2, 2])] == under {
             // diag R to L
             // 13, 31 -> 22
             tempRC = [2, 2]
             // proposeFlag = true
         } else if game[pos(pt1: [grd12, 2, 2])] == tok12 && game[pos(pt1: [grd12, 3, 1])] == tok12 && game[pos(pt1: [grd12, 1, 3])] == under {
             // diag R to L
             // 22, 31 -> 13
             tempRC = [1, 3]
             // proposeFlag = true
